...
function showTitle(title) {
    resetModel();
    document.getElementById("techniqueTitle").textContent = title;

    // Ocultar los controles del modelo
    document.getElementById('modelControls').style.display = 'none';

    // Mostrar controles dependiendo de la técnica seleccionada
    if (title === 'Regresión Lineal' || title === 'Regresión Polinomial' || title === 'Árbol de Decisión' || title === 'Red Neuronal' || title === 'KMeans') {
        document.getElementById('modelControls').style.display = 'block';
        // Eliminar la siguiente línea para evitar mostrar el título dos veces
        // document.getElementById('modelTitle').innerText = title; // Set the model title
    }

    const inputContainer = document.getElementById("inputContainer");
    inputContainer.innerHTML = "";
    let percentSelectGroup, predictionRangeGroup, decisionTreeGroup;

    let inputField;
    switch (title) {
        case "Regresión Lineal":
            console.log("Regresión Lineal");
            inputField = createFileInput("Cargar datos para Regresión Lineal (CSV)");
            tipoDeModelo = "linear-regression";
            inputField.addEventListener("change", () => handleFileUpload(inputField.files[0], title));
            inputContainer.appendChild(inputField);

            // Agregar campos específicos para Regresión Lineal
            percentSelectGroup = document.createElement("div");
            percentSelectGroup.className = "form-group mt-3";
            percentSelectGroup.innerHTML = `
            ...
            `;
            inputContainer.appendChild(percentSelectGroup);

            predictionRangeGroup = document.createElement("div");
            predictionRangeGroup.className = "form-group mt-3";
            predictionRangeGroup.innerHTML = `
            ...
            `;
            inputContainer.appendChild(predictionRangeGroup);
            break;

        case "Regresión Polinomial":
            inputField = createFileInput("Cargar datos para Regresión Polinomial (CSV)");
            tipoDeModelo = "polynomial-regression";
            inputField.addEventListener("change", () => handleFileUpload(inputField.files[0], "polynomial-regression"));
            inputContainer.appendChild(inputField);
            ...
            // Mostrar el selector de grado para la regresión polinomial
            const degreeSelectGroup = document.createElement("div");
            degreeSelectGroup.className = "form-group mt-3";
            degreeSelectGroup.innerHTML = `
                    <label for="degree-select">Selecciona el grado de la Regresión Polinomial:</label>
                    <input type="number" id="degree-select" class="form-control" value="2" min="1" max="10">
                `;
            inputContainer.appendChild(degreeSelectGroup);
            predictionRangeGroup = document.createElement("div");
            predictionRangeGroup.className = "form-group mt-3";
            predictionRangeGroup.innerHTML = `
                <label for="prediction-range">Rango para Nueva Predicción (X):</label>
        ...
        case "Árbol de Decisión":
            tipoDeModelo = "decision-tree";
            inputField = createFileInput("Cargar datos para Árbol de Decisión (CSV)");
            inputField.addEventListener("change", () => handleFileUpload(inputField.files[0], "decision-tree"));

            // Agregar campos específicos para Árbol de Decisión
            decisionTreeGroup = document.createElement("div");
            decisionTreeGroup.className = "form-group mt-3";
            decisionTreeGroup.innerHTML = `
                <label for="data">Predict:</label>
                <input type="text" id="data" class="form-control" placeholder="Medium,Short,No,Yes,No">
            `;
            inputContainer.appendChild(decisionTreeGroup);

            break;

        case "Red Neuronal":
            tipoDeModelo = "neuronal-network";
            inputField = createFileInput("Cargar datos para Red Neuronal (CSV)");
            inputField.addEventListener("change", () => handleFileUpload(inputField.files[0], "neuronal-network"));
            break;

        case "KMeans":
            tipoDeModelo  = "k-means-linear";
            inputField = createFileInput("Cargar datos para KMeans (CSV)");
            inputField.addEventListener("change", () => handleFileUpload(inputField.files[0], "k-means-linear"));
            break;

        default:
            inputField = document.createElement("p");
            inputField.textContent = "Seleccione una técnica válida.";
    }
...
function handleFileUpload(file, modelType) {
    ...
    reader.onload = function (event) {
        ...
        processCSVData(csvData, modelType);
    };
    reader.readAsText(file);
}

function processCSVData(data, modelType) {
    ...
    if (modelType === "Regresión Lineal") {
        ...
        arrNR = lines[0].split(',').map(Number);
